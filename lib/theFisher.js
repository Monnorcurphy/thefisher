...
function checkChildren(){
  document.fish.innerHTML= `Fish Remaining: ${document.game.fisher.checkKids()}`
}
...
function updateAccuracy(){
  ...
  if (document.game.fisher.checkKids() === document.total){
    rate = 0
  } else{
    rate =(document.total - document.game.fisher.checkKids())/document.count;
  }
  if(!rate){
    rate = 1
  }
  accuracy.innerHTML = `Accuracy: ${rate * 100}%`
}

function updateSpeed(){
  fishRemaining = document.total -document.game.fisher.checkKids()
  if (document.total){
    if(fishRemaining === 0){
...
document.addEventListener("DOMContentLoaded", function(){
  ...
  function reset (){
    ...
    document.count = 0
    document.total = document.game.fisher.checkKids()
    victory.innerHTML = ''
